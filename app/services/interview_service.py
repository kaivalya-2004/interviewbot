...
class InterviewService:
    ...
    def start_new_interview(self, resume_text: str, candidate_id: str, questionnaire: List[str]) -> str:
        ...
        if questionnaire: questionnaire_str = "\n".join([f"- {q}" for q in questionnaire]); questionnaire_context = f"\n**Reference Questionnaire:**\n---\n{questionnaire_str}\n---"
        else: questionnaire_context = "\n**Reference Questionnaire:** Not provided."
        system_prompt = f"""You are an AI Interviewer... (rest of prompt remains same) ... {resume_text} --- {questionnaire_context} **Task:** ...""" # Assuming prompt is correct
        try:
            ...
            self.active_chat_sessions[session_id] = chat_model.start_chat(history=[ {'role': 'user', 'parts': [system_prompt]}, {'role': 'model', 'parts': ["Understood. Ready after the candidate introduces themselves."]} ])
            logger.info(f"✅ Gemini chat session initialized for {session_id}")
        except Exception as e:
            # Clean up on failure
    ...
    # --- MODIFIED: Added Camera/Mic Instructions ---
    ...
    def _save_audio_thread( self, audio_data: np.ndarray, sample_rate: int, audio_path: str, session_id: str, text: str, is_follow_up: bool ):
        try: sf.write(str(audio_path), audio_data, sample_rate); self.db.add_message_to_session(session_id, "assistant", text, str(audio_path), is_follow_up=is_follow_up); logger.info(f"💾 BG save ok: {audio_path}")
        except Exception as e: logger.error(f"❌ BG audio save fail {audio_path}: {e}")
    ...
    def text_to_speech( self, text: str, session_id: str, turn_count: int, candidate_id: str, is_follow_up: bool = False ) -> Tuple[Optional[np.ndarray], Optional[int]]:
        if not self.tts_client: logger.error("TTS client N/A."); return None, None
        try:
            ...
            synthesis_input = texttospeech.SynthesisInput(text=text); char_count = len(text)
            self._session_tts_char_counts[session_id] += char_count
            logger.info(f"Generating TTS audio ({char_count} chars){' (followup)' if is_follow_up else ''}: '{text[:50]}...'")
            ...
            save_thread = threading.Thread( target=self._save_audio_thread, args=(audio_data, sample_rate, audio_path, session_id, text, is_follow_up), daemon=True ); save_thread.start()
            return audio_data, sample_rate
        except Exception as e: logger.error(f"TTS gen fail: {e}", exc_info=True); return None, None

    def speech_to_text(self, audio_path: str) -> str:
        if not Path(audio_path).exists(): logger.error(f"STT file missing: {audio_path}"); return "[Error: Audio file missing]"
        with sr.AudioFile(audio_path) as source:
            try: logger.debug(f"STT record: {audio_path}..."); audio_data = self.recognizer.record(source); logger.debug("STT recognize...")
            except Exception as e: logger.error(f"STT record fail: {e}"); return "[Error recording segment]"
            try: text = self.recognizer.recognize_google(audio_data); logger.info(f"STT ok: '{text}'"); return text
            except sr.UnknownValueError: logger.warning("STT unclear."); return "[Unintelligible]"
            except sr.RequestError as e: logger.error(f"STT service err: {e}"); return "[Speech service error]"
            except Exception as e: logger.error(f"STT unexpected err: {e}", exc_info=True); return "[STT Error]"
    ...
    def generate_next_question(self, session_id: str) -> str:
        ...
        if not chat: logger.error(f"No active chat for {session_id}."); return "Error: Session state lost."
        with self._transcript_lock: last_user_answer = self._latest_transcript_for_gemini.get(session_id, "[Error retrieving last answer]")
        try:
            ...
            try: # Log tokens
                ...
                if metadata:
                    prompt_tokens = getattr(metadata, 'prompt_token_count', 0); response_tokens = getattr(metadata, 'candidates_token_count', 0); total_tokens = getattr(metadata, 'total_token_count', 0)
                    self._session_token_counts[session_id]['prompt'] += prompt_tokens; self._session_token_counts[session_id]['response'] += response_tokens; self._session_token_counts[session_id]['total'] += total_tokens
                    logger.debug(f"[Turn Tokens] P:{prompt_tokens}, R:{response_tokens}, T:{total_tokens}")
                else: logger.warning("No usage_metadata in Gemini response.")
            except Exception as e: logger.error(f"Error logging Gemini tokens: {e}")
            logger.info(f"Generated Q: '{next_question[:100]}...'")
            if not next_question or len(next_question) < 5: logger.warning("Gemini Q short/empty, fallback."); return "Tell me more about a project you're proud of."
            return next_question
        except Exception as e: logger.error(f"Gemini generate fail: {e}", exc_info=True); return "Apologies, error occurred. What's a key skill?"
    ...
    def process_and_log_transcript(
        ...
        try:
            ...
            try: self.db.add_message_to_session(session_id, "user", transcript, audio_path, start_time, end_time, is_follow_up=is_follow_up_response) # Log even on error
            except Exception as db_e: logger.error(f"(BG Thread) Failed log audio error to DB: {db_e}")

    def generate_final_transcript_file(self, session_id: str) -> None:
        """Generates text transcript file."""
        session_data = self.db.get_session(session_id)
        if not session_data: logger.error(f"No session data for transcript: {session_id}"); return
        candidate_id = session_data.get("candidate_id", "unknown")
        transcript_dir = Path("data") / candidate_id / "transcripts"; transcript_dir.mkdir(parents=True, exist_ok=True)
        transcript_path = transcript_dir / f"transcript_{session_id}.txt"
        try:
            with open(transcript_path, "w", encoding="utf-8") as f:
                f.write(f"--- Interview Transcript ---\nSession ID: {session_id}\nCandidate ID: {candidate_id}\n")
                created_at = session_data.get('created_at'); f.write(f"Date: {created_at.strftime('%Y-%m-%d %H:%M:%S')}\n" if created_at else "Date: N/A\n")
                ...
                for msg in session_data.get("conversation", []):
                    role = msg.get("role", "unknown").capitalize(); text = msg.get("text", "")
                    ts = msg.get("timestamp"); ts_str = ts.strftime('%Y-%m-%d %H:%M:%S') if ts else "N/A"
        ...
        except Exception as e: logger.error(f"Failed write transcript: {e}", exc_info=True)
    ...
    def end_interview_session(self, session_id: str):
        ...
        else: logger.warning(f"No token count data for session {session_id} on end.")
        # Log TTS Characters
        ...
        else: logger.warning(f"No TTS char count data for session {session_id} on end.")
        # Clear Chat Session
        if session_id in self.active_chat_sessions:
            del self.active_chat_sessions[session_id]; logger.info(f"Cleared Gemini chat session {session_id}")
        else: logger.warning(f"End session {session_id}, but no active chat.")
        # Clear latest transcript state
        with self._transcript_lock:
            if session_id in self._latest_transcript_for_gemini:
                del self._latest_transcript_for_gemini[session_id]; logger.info(f"Cleared transcript state for {session_id}")