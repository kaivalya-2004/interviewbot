...
class MeetSessionManager:
    ...
    def start_bot_session( self, session_id: str, meet_link: str, candidate_id: str, audio_device: Optional[int] = None, enable_video: bool = True, headless: bool = True, video_capture_method: str = "javascript" ) -> bool:
        try:
            ...
            if not controller.setup_driver(): 
                logger.error(f"❌ Failed setup driver {session_id}"); return False

            # --- Device validation logic REMOVED ---

            bot_name = f"AI Interviewer Bot"
            if not controller.join_meeting(meet_link, bot_name): 
                logger.error(f"❌ Failed join meeting {session_id}"); controller.cleanup(); return False
            
            logger.info(f"✅ Bot joined meeting: {session_id}")

            # --- Tab monitoring injection REMOVED ---
            ...
            self.active_sessions[session_id] = { 
                ...
                # --- tab_switch_events and device_type REMOVED ---
                'capture_stats': { 'total_attempts': 0, 'successful_captures': 0, 'failed_captures': 0, 'js_captures': 0, 'screenshot_captures': 0 } 
            }
            return True
        except Exception as e: 
            logger.error(f"❌ Error starting bot session: {e}", exc_info=True); return False
    ...
    def _start_candidate_video_capture(self, session_id: str):
        ...
        def capture_loop():
             ...
             logger.info("Waiting 5s for video stream..."); time.sleep(5) 
             while not stop_event.is_set():
                 try:
                     current_session = self.active_sessions.get(session_id)
                     if not current_session: logger.warning("Session ended, stopping capture loop."); break
                     current_controller = current_session.get('controller')
                     ...
                     if current_time - last_capture_time >= capture_interval:
                         ...
                         stats['total_attempts'] += 1; screenshot_data = None; width = 0; height = 0 
                         if capture_method == "javascript":
                             ...
                             if result: 
                                 ...
                                 logger.debug(f"JS capture ok ({width}x{height})")
                                 
                                 # --- Storing video resolution REMOVED ---
                         ...
                         if screenshot_data:
                             ...
                             if current_session['snapshot_count'] % 5 == 0: 
                                 method_used = "JS" if stats['js_captures'] >= stats['screenshot_captures'] else "Scr"; 
                                 logger.info(f"📸 Snaps: {current_session['snapshot_count']} ({method_used}) | Rate: {stats['successful_captures']}/{stats['total_attempts']}")
            ...
            try:
                ...
                if participant_count == 2:
                    ...
                    if consecutive_joins >= required_stable_checks:
                        ...
                        # --- Mobile device/ratio check REMOVED ---
    ...
    def get_all_active_sessions(self) -> Dict[str, Dict[str, Any]]:
        ...
        for sid, session in active_sessions_copy.items():
             ...
             summary[sid] = { 
                ...
                'video': session.get('video_enabled'), 
                'method': session.get('video_capture_method'), 
                'snaps': session.get('snapshot_count', 0),
                # --- tab_switches and device_type REMOVED ---
